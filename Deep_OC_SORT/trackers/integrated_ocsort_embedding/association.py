...
def split_distance_affinity(dets, trks, affinity_thresh=0.55, pair_diff_thresh=0.6, hard_thresh=True):
    eucl_dist = np.zeros((len(dets), len(trks)))
    ...
    for i in range(len(dets)):
        for j in range(len(trks)):
            ...
            # Euclidean Distance
            eucl_d = sp.distance.cdist(dets[i], trks[j], "euclidean")  # Euclidean Distance
            patch_distance = np.min(eucl_d, axis=0)  # 최소 거리 선택 (가장 유사한 것)

            # Euclidean Distance를 Cosine Similarity와 비슷한 스케일로 변환
            eucl_affinity = np.exp(-patch_distance)  # 값이 작을수록 유사함 → exp(-x) 변환

            # Hard Threshold 적용
            if hard_thresh:
                if len(np.where(eucl_affinity > affinity_thresh)[0]) != len(eucl_affinity):
                    eucl_dist[i, j] = 0
                else:
                    eucl_dist[i, j] = np.max(eucl_affinity)
            else:
                eucl_dist[i, j] = np.max(eucl_affinity)  # Max or Mean 가능
...
def associate(
    ...
    if not emb_off:
        if grid_off:
            ...
            # emb_cost =  -euclidean_distance(det_embs, trk_embs)#-split_distance_affinity(det_embs, trk_embs)
    ...
    if min(iou_matrix.shape) > 0:
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            if not aw_off:
                ...
                emb_cost *= w_matrix
                # emb_cost = -np.exp(-emb_cost) * w_matrix  # 거리 기반의 적절한 가중치 변환

            else:
                emb_cost *= w_assoc_emb
                # emb_cost = -np.exp(-emb_cost) * w_assoc_emb  # 거리 기반 변환
