...
import sys
# sys.path.append('/Users/horang/Documents/ai/MOT/Deep_OC_SORT/')
base_path = '/Users/horang/Documents/ai/MOT/Deep_OC_SORT/'
class CMCComputer:
    def __init__(self, minimum_features=10, method="file"):
        assert method in ["file", "sparse", "sift"]

        os.makedirs(base_path+"cache", exist_ok=True)
        self.cache_path = base_path+"cache/affine_ocsort.pkl"
        self.cache = {}
        ...
        # Same BoT-SORT CMC arrays
            ...
            # DanceTrack
            for f_name in os.listdir(base_path+"cache/cmc_files/DanceTrack/"):
                tag = f_name.replace("GMC-", "").replace(".txt", "").replace("-", "")
                f_name = os.path.join(base_path+"cache/cmc_files/DanceTrack/", f_name)
                self.file_names[tag] = f_name

            # All the ablation file names
            for f_name in os.listdir(base_path+"cache/cmc_files/MOT17_ablation/"):
                # The tag that'll be passed into compute_affine based on image name
                tag = f_name.replace("GMC-", "").replace(".txt", "") + "-FRCNN"
                f_name = os.path.join(base_path+"cache/cmc_files/MOT17_ablation/", f_name)
                self.file_names[tag] = f_name
            for f_name in os.listdir(base_path+"cache/cmc_files/MOT20_ablation/"):
                tag = f_name.replace("GMC-", "").replace(".txt", "")
                f_name = os.path.join(base_path+"cache/cmc_files/MOT20_ablation/", f_name)
                self.file_names[tag] = f_name

            # All the test file names
            for f_name in os.listdir(base_path+"cache/cmc_files/MOTChallenge/"):
                tag = f_name.replace("GMC-", "").replace(".txt", "")
                if "MOT17" in tag:
                    tag = tag + "-FRCNN"
                ...
                if tag in self.file_names:
                    continue
                f_name = os.path.join(base_path+"cache/cmc_files/MOTChallenge/", f_name)
                self.file_names[tag] = f_name
